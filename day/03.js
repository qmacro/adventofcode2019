const { curry, apply, add, minBy, divide, __, lift, pluck, max, intersection, reduce, inc, dec, last, range, concat, equals, juxt, compose, head, tail, map, split } = require('ramda')
...
const nearestCrossoverToZero = ([trace1, trace2]) => {
  const
    path1 = pathCoords([0,0], parseTrace(trace1)),
    path2 = pathCoords([0,0], parseTrace(trace2))
    crossovers = intersection(path1, path2)
    nearest = reduce(minBy(sumAbsCoords), [Infinity,Infinity], tail(crossovers))
